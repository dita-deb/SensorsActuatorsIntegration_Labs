...
int gametime = 60;                   // Total game time (60 seconds)
const int maxTime = 60;              // Maximum game time (60 seconds)
bool isGreen = true;                 // light phases (Green or Red)
int lastDistance = 0;                // Store the last measured distance (for motion detection)
const int motionTolerance = 5;       // Motion detection tolerance (in cm)

// Random duration ranges for green and red lights (in milliseconds)
...
// Victory condition: player wins if they are within 5 cm of the sensor
...
void setup() {
    ...
    lcd.begin();   // Start LCD without parameters (address assumed)
    lcd.backlight();
    lcd.setCursor(0, 0);
    lcd.print("Press to Start"); // Display starting message
    ...
    pinMode(greenLedPin, OUTPUT);         // Green LED pin set as output
    pinMode(redLedPin, OUTPUT);           // Red LED pin set as output
    pinMode(ultrasonicTriggerPin, OUTPUT); // Ultrasonic trigger pin as output
    pinMode(ultrasonicEchoPin, INPUT);    // Ultrasonic echo pin as input

    // Attach servos to their respective pins
    ultraServo.attach(33);     // Attach ultrasonic servo to pin 33
    eliminationServo.attach(31); // Attach elimination arm servo to pin 31
...
void gameLoop() {
    ...
    if (buttonPressed) {
        ...
        previousMillis = millis(); // Reset timer to current time
...
void displayTime() {
    ...
    lcd.print("Time Left: ");     // Print "Time Left:" label
    lcd.print(gametime);         // Print the remaining time
    lcd.print("s ");             // Print "s" to indicate seconds
}
...
void greenLight() {
    ...
    tone(buzzerPin, 1000);  // Start buzzer with 1000 Hz frequency
    delay(400);              // Buzzer on for 400ms
...
void redLight() {
    ...
    lcd.setCursor(0, 1);         // Set cursor to the second row of the LCD
    lcd.print("Dist: ");          // Print "Dist: " label
    lcd.print(distance);         // Print the measured distance
    lcd.print("cm");             // Print unit (cm)

    digitalWrite(greenLedPin, LOW); // Turn off green LED
    digitalWrite(redLedPin, HIGH);  // Turn on red LED

    // Activate the buzzer for a short duration (50ms)
    tone(buzzerPin, 800);  // Start buzzer with 800 Hz frequency
...
int getDistance() {
    digitalWrite(ultrasonicTriggerPin, LOW);  // Clear the trigger
    delayMicroseconds(2);                     // Wait 2 microseconds
    digitalWrite(ultrasonicTriggerPin, HIGH); // Send trigger signal
    delayMicroseconds(10);                    // Wait 10 microseconds
    digitalWrite(ultrasonicTriggerPin, LOW);  // Stop trigger

    long duration = pulseIn(ultrasonicEchoPin, HIGH); // Measure echo time
    return duration * 0.034 / 2; // Convert the duration to cm (speed of sound is 0.034 cm/µs)
}
